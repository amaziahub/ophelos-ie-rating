...
INVALID_STATEMENT_ID = 999

VALID_STATEMENT_ID = 1

INVALID_USER_ID = 999
FIRST_VALID_USER_ID = 1
SECOND_VALID_USER_ID = 2
...
def test_submit_new_statement(app):
    response = app.submit_statement(build_statement(FIRST_VALID_USER_ID))
...
def test_retrieve_statement(app):
    ...
    report = app.get_statement(statement_id, FIRST_VALID_USER_ID)

    assert_that(report["id"], equal_to(statement_id))
    assert_that(report["user_id"], equal_to(FIRST_VALID_USER_ID))
...
def test_unable_to_retrieve_statement_of_different_user(app):
    ...
    with pytest.raises(requests.HTTPError) as exc_info:
        app.get_statement(statement_id, SECOND_VALID_USER_ID)
...
def test_calculate_ie_rating_success(app):
    ...
    rating_response = app.get_rating(statement_id, FIRST_VALID_USER_ID)
...
def test_calculate_ie_rating_user_not_found(app):
    with pytest.raises(requests.HTTPError) as exc_info:
        app.get_rating(VALID_STATEMENT_ID, INVALID_USER_ID)
...
def test_calculate_ie_rating_statement_not_found(app):
    with pytest.raises(requests.HTTPError) as exc_info:
        app.get_rating(INVALID_STATEMENT_ID, FIRST_VALID_USER_ID)
    assert_that(exc_info.value.response.status_code, is_(404))
...
def test_calculate_period_rating(app):
    ...
    app.submit_statement(build_statement(FIRST_VALID_USER_ID))
    app.submit_statement(build_statement(FIRST_VALID_USER_ID))

    response = app.get_rating_period(FIRST_VALID_USER_ID,
